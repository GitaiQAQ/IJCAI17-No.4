...
- 测评函数：<img src="http://www.forkosh.com/mathtex.cgi? L = \frac{1}{{NT}}\sum\limits_i^N {\sum\limits_t^T {\left| {\frac{{{p_{it}} - {r_{it}}}}{{{p_{it}} + {r_{it}}}}} \right|} } ">,其中$p_{it}$为商家$i$在日期$t$销量预测值，$r_{it}$为其真实值。
